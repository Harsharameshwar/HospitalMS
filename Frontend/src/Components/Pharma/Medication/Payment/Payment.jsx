...
export default function Payment() {
  ...
  const navigate = useNavigate()
    ...
    let crum = ['Home','Medication', 'Payment-1'];
  ...
  return (
    <>
        ...
        <div className='ml-[80%] mt-4 mb-5 pb-5'>
          <Button size={SIZE.default} disabled={dis} shape={SHAPE.square} onClick={e=>navigate('/pharma/medication/payment/2', {state:{data:pays,id:state.data._id}})} overrides={{ BaseButton: { style: ({ $theme }) => ({  width: "8rem", }) } }} > <ArrowRight size={40}/> </Button>
        </div>
    </>
  )
}
